...
public class List_v0 extends BaseCase {
	@Test(summary = "获取最新视频列表(降序)", expectedResults = "返回结果格式正确", index = 1)
	...
	@Test(summary = "获取最新视频列表(升序)", expectedResults = "返回结果格式正确", index = 2)
	...
	@Test(summary = "最新视频加载更多", expectedResults = "返回结果格式正确", index = 3)
	public void testFileList_LoadNew() {
		g_user.setHttpParam("category", "weichat");
		g_user.setHttpParam("pageSize", "10");
		g_user.setHttpParam("orderBy", "desc");
		g_user.setHttpParam("startKey", "25438252677990400");
	...
	@Test(summary = "主页获取最新视频", expectedResults = "返回结果格式正确", index = 4)
